...
pub struct PassthroughFS {
    pub target: OsString,
    pub log: std::sync::Mutex<io::BufWriter<File>>,
}
            ...
            match libc_wrappers::readdir(fh) {
                Ok(Some(entry)) => {
                    ...
                    let filetype = match entry.d_type {
                        ...
                        libc::DT_SOCK => {
                            warn!(
                                "FUSE doesn't support Socket file type; translating to NamedPipe instead."
                            );
                            FileType::NamedPipe
                        }
                        _ => {
        ...
        match file.read(unsafe {
            ...
            Ok(n) => {
                ...
                if n > 0 {
                    ...
                    let _ = writeln!(log, "{}\t{}\t{}", Path::new(&real).display(), offset, n);
                }
            }
            Err(e) => {
